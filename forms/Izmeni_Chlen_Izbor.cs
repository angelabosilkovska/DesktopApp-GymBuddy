...
{
    public partial class Izmeni_Chlen_Izbor : Form
    {
        public Izmeni_Chlen_Izbor()
        {
            InitializeComponent();
        }
        ...
        private void Izmeni_Chlen_Izbor_Load(object sender, EventArgs e)
        {
            this.Width = 1000;
            this.Height = 600;
            this.ShowIcon = false;
            ...
            conn.Open();
            dataAdapter = new SqlDataAdapter("select IDBroj AS РеденБрој, Ime AS Име, Prezime AS Презиме, Pol AS Пол, [Datum Rag] AS ДатумНаРаѓање, Adresa AS Адреса, Telefon As Телефон from Chlenovi", conn);
        ...
        public void Zachuvaj_Click(object sender, EventArgs e)
        {
             ...
             {
                    conn.Open();
                    string query = "Update Chlenovi set Prezime=@tbNovoPrezime where IDBroj = @tbrb";
             ...
             {
                    conn.Open();
                    string query = "Update Chlenovi set Adresa=@tbNovaAdresa where IDBroj = @tbrb";
                    SqlCommand cmd = new SqlCommand(query, conn);
                    cmd.Parameters.Add("@tbrb", SqlDbType.Int).Value = Convert.ToInt32(tbrb.Text);
             ...
             {
                    conn.Open();
                    string query = "Update Chlenovi set Telefon=@tbNovTelefon where IDBroj = @tbrb";
                    SqlCommand cmd = new SqlCommand(query, conn);
                    cmd.Parameters.Add("@tbrb", SqlDbType.Int).Value = Convert.ToInt32(tbrb.Text);
