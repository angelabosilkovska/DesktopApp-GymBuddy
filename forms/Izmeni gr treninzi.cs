...
{
    public partial class Izmeni_gr_treninzi : Form
    {
        public Izmeni_gr_treninzi()
        {
            InitializeComponent();
        }
        ...
        private void Izmeni_gr_treninzi_Load(object sender, EventArgs e)
        {
            this.Width = 1000;
            this.Height = 600;
            this.ShowIcon = false;
            ...
            lb1.Text = "Измени Групен Тренинг";
        ...
        public void Zachuvaj_Click(object sender, EventArgs e)
        {
            ...
            {
                conn.Open();
                string query = "Update Grupni_Treninzi set Ime=@tbNovoIme where ID = @tbrb";
          ...
          if (tbNovaCena.Text != "")
            {
                conn.Open();
                string query = "Update Grupni_Treninzi set Cena=@tbNovaCena where ID = @tbrb";
                SqlCommand cmd = new SqlCommand(query, conn);
                cmd.Parameters.Add("@tbrb", SqlDbType.Int).Value = Convert.ToInt32(tbrb.Text);
