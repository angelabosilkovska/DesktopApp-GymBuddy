...
{
    public partial class Izmeni_Suplementi : Form
    {
        public Izmeni_Suplementi()
        {
            InitializeComponent();
        }
        ...
        private void Izmeni_Suplementi_Load(object sender, EventArgs e)
        {
            this.Width = 1000;
            this.Height = 600;
            this.ShowIcon = false;
        ...
        public void Zachuvaj_Click(object sender, EventArgs e)
        {
            ...
            {
                conn.Open();
                string query = "Update Suplementi set Ime=@tbNovoIme where ID = @tbrb";
           ...
           if (tbNovaCena.Text != "")
            {
                conn.Open();
                string query = "Update Suplementi set Cena=@tbNovaCena where ID = @tbrb";
                SqlCommand cmd = new SqlCommand(query, conn);
                cmd.Parameters.Add("@tbrb", SqlDbType.Int).Value = Convert.ToInt32(tbrb.Text);
        ...
        {
            this.Hide();
            Суплементи suplemeti= new Суплементи();
            suplemeti.FormClosed += (s, args) => this.Close();
            suplemeti.Show();
        }
